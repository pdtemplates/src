...
function imageMapper(index, dataUri, imageMap) {
	...
	var info = (function() {
		var form = utils.id('edit_details'),
			...
			plink_attr = utils.id('plink_attr'),
	...
	//	delete_button.addEventListener('click', deleteArea, false);
		...
		plink_attr.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
		...
		plink_attr.addEventListener('change', change, false);
		...
		return {
			...
			//	console.log(plink_attr);
				//obj.plink = jQuery( plink_attr ).prop( "checked" )
				plink_attr.value = object.plink ? object.plink : '';
				utils.show(form);
				if (new_x && new_y) {
					x = new_x;
					y = new_y;
		...
		function test(str) {
			...
			if (str) {
				...
				while (result_area) {
					...
					result_plink = regexp_plink.exec(area);
					if (result_plink) {
						plink = result_plink[1];
					} else {
						plink = '';
					}
	...
	var Rect = function (x, y){
		...
		this.plink = 'false'; //title attribute - not required
